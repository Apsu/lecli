...
def test_disable_api_key(mocked_url, mocked_owner_apikey, mocked_owner_apikey_id,
    ...
    out, err = capsys.readouterr()
    assert {'apikey': {'active': False}} == json.loads(httpretty.last_request().body.decode('utf8'))
    assert not err
...
def test_enable_api_key(mocked_url, mocked_owner_apikey, mocked_owner_apikey_id,
    ...
    out, err = capsys.readouterr()
    assert {'apikey': {'active': True}} == json.loads(httpretty.last_request().body.decode('utf8'))
    assert not err
